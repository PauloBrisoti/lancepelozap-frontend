...
export function ClientesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* No mobile o cadastro vira uma página normal no fluxo do documento —
          a rolagem da página (que sempre funciona) leva até os botões. */}
      {telaCadastro && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between bg-gray-50 rounded-t-xl">
            <button type="button" onClick={() => setTelaCadastro(false)} className="text-brand-600 font-medium text-sm">← Voltar</button>
            <h3 className="text-base font-bold text-gray-900">{formulario.id ? 'Editar Cliente' : 'Novo Cliente'}</h3>
          </div>
          <form onSubmit={handleSalvar} className="p-4 space-y-3">
            {renderCamposFormulario()}
            <div className="flex gap-3 pt-3 border-t border-gray-100">
              <button type="button" onClick={() => setTelaCadastro(false)} className="flex-1 px-4 py-3 rounded-lg font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 transition text-sm">Cancelar</button>
              <button type="submit" className="flex-1 px-4 py-3 rounded-lg font-medium text-white bg-brand-600 hover:bg-brand-700 transition shadow-sm text-sm">Salvar Cliente</button>
            </div>
          </form>
        </div>
      )}
